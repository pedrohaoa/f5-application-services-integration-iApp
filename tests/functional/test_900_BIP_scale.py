...
def remove_application_service(
    ...
    for removal_no in range(deployment_no + 1):
        ...
        try:
            ...
            test_results[strip_payload_name(payload['name'])].setdefault('removal_exception',[]).append((removal_no, str(ex)))
            logger.exception(ex)
            if fail_fast:
                raise
...
def independent_scale(payload, first_pool_addr, log_dir, bip_client,
    ...
    for deployment_no in range(scale_size):
        ...
        try:
            ...
            test_results[strip_payload_name(payload['name'])].setdefault('completed', []).append(deployment_no + 1)

        except (AppServiceDeploymentException, RESTException,
                AppServiceDeploymentVerificationException, ValueError) as ex:
            test_results[strip_payload_name(payload['name'])].setdefault('deployment_exception', []).append((deployment_no, str(ex)))
            logger.exception(ex)
